...
assert.equal((page.match(/\/downloads\/類似会社選定ワークシート\.xlsx/g) ?? []).length, 2, "worksheet CTA must appear at article top and end");
...
assert.ok(sitemap.includes('item.path === "/comps-peer-selection" ? new Date("2026-07-22T00:00:00+09:00")'), "reviewed route must use its 2026-07-22 lastModified date");

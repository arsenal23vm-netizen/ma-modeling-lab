...
for (const label of ["売上高", "成長率", "EBITDAマージン", "メンテナンス・サービス比率"]) {
  assert.equal((cards.match(new RegExp(label, "g")) ?? []).length, 4, `each comparison card shows ${label}`);
}
assert.match(cards, /成長率<\/dt><dd>該当なし<\/dd>/, "target growth is explicitly unavailable");

const funnel = renderToStaticMarkup(<SelectionFunnel />);
for (const [label, count] of [["業種候補", 30], ["事業モデル", 18], ["財務比較", 12], ["主要比較会社", 6]]) {
...
for (const label of ["主要比較会社", "補完比較会社", "目標比較会社", "非比較会社", "類似するが除外", "参考会社"]) {
...
assert.equal((closeRow[0].match(/>該当なし<\/td>/g) ?? []).length, closePeer.dataGaps.length, "only unavailable criteria render 該当なし");
...
assert.match(matrix, /類似するが除外/, "matrix uses the Japanese close-exclusion role label");
assert.match(matrix, /参考会社/, "matrix uses the Japanese limited-comparison role label");
for (const label of ["主要比較会社", "補完比較会社", "目標比較会社", "非比較会社"]) {

...
export const candidatePeers: CandidatePeer[] = [
  ...
  { id: "close", name: "中央FAテクノロジー", business: "FA装置・部品", geography: "日本", revenue: 1080, growth: 7, ebitdaMargin: null, serviceMix: 29, capitalIntensity: "中程度", customerMarket: "電子部品・一般製造業", accounting: "日本基準", dataAvailable: false, role: "excluded_close_peer", criticalMismatch: true, rationale: "事業構成、顧客、売上規模は非常に近いが、EBITDA情報が取得不能で利益率比較と倍率検証に使えない。", scores: scores(3, 3, 3, 3, 3, 3, 0, 3, 3, 2, 3, 0) },
...
export const peerSelectionFaqs: [string, string][] = [
  ["スコアが高ければCore Peerにできますか？", "いいえ。重大な不一致、開示の質、事業の純粋性をレビューし、合計点だけで自動採用しません。"],
  ["規模が大きく異なる企業は除外すべきですか？", "必ずしも除外しません。事業モデルが近ければSecondary Peerとして参照し、規模差による影響を明記します。"],
  ["EBITDA情報がない近似企業は使えますか？", "事業理解の参考にはできますが、利益率やマルチプルの比較には使わず、除外理由を記録します。"],
  ["同じセクターならComparableと考えてよいですか？", "いいえ。セクター分類はLong Listを作る出発点です。収益モデル、製品ミックス、顧客市場、資本集約度、循環性まで確認して比較可能性を判断します。"],
  ["中央値をそのまま採用してよいですか？", "いいえ。中央値は候補群の構成に左右されます。Core Peerを中心に、除外・補助比較の理由、各社の規模差や一過性要因を確認して採用レンジを決めます。"],
];

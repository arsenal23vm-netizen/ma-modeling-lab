...
const canonical = "https://arsenal23vm-netizen.github.io/ma-modeling-lab/valuation/dcf/sensitivity-analysis";
...
export default function SensitivityAnalysisPage() {
  ...
  return (
    <DcfLessonShell number="04" href="/valuation/dcf/sensitivity-analysis" title="DCF感応度分析の作り方――WACCと永久成長率を並べる" lead="単一の評価額を答えにせず、WACCとTerminal Growthの組合せがEnterprise Valueへ与える影響を5×5表で確認します。" readingTime="約12分" sections={sections} previous={{ href: "/valuation/dcf/terminal-value", label: "03 継続価値を計算する" }} next={{ href: "/valuation/dcf/enterprise-to-equity", label: "05 EVを株主価値へつなぐ" }}>
      <h2 id="definition">定義と使う場面</h2>
      <p>感応度分析は、重要な評価前提を一定幅で動かし、結論がどこまで変わるかを確認するレビュー手続です。このケースではWACCを行、Terminal Growthを列に置きます。</p>
      <div className="formula">WACC × Terminal Growth</div>
      <p>WACCが上がると企業価値は下がり、永久成長率が上がると企業価値は上がります。表の上下左右がこの方向性と反対なら、Data Tableの行入力セル・列入力セルまたは参照先を確認します。</p>
      ...
      <div className="callout"><strong>無効セルのガード</strong><br />各セルでWACC &gt; gを確認し、WACC ≤ gなら計算結果を表示せずN/Aにします。エラー値や極端な正数を評価レンジに混ぜません。</div>
      ...
      <DcfCommonErrors errors={[{ title: "行・列の入力セルを逆にする", body: "上段のgは行入力セル、左列のWACCは列入力セルへ対応させます。逆にすると方向性チェックで異常が見つかります。" }, { title: "WACC ≤ gを数値表示する", body: "Gordon Growth法の分母がゼロ以下のセルは経済的に解釈できません。IFガードでN/Aとし、評価レンジから除外します。" }]} />
      <DcfReviewCheck>右方向へ移動すると企業価値が上がり、下方向へ移動すると下がること、全25セルの単位が百万円で、無効条件がN/Aになることを確認します。</DcfReviewCheck>
    </DcfLessonShell>
  );
}

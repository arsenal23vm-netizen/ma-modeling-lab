...
export const metadata: Metadata = {
  title: "継続価値の計算方法｜Terminal Valueと構成比を検証する",
  description: "Gordon Growth法でFCFF(n+1)、WACC、永久成長率から継続価値を計算し、期末割引と企業価値に占める比率を確認します。",
  alternates: { canonical },
  openGraph: { title: "継続価値の計算方法｜Terminal Valueと構成比を検証する", description: "共有ケースで継続価値と価値集中を確認する教育用レッスン。", url: canonical, type: "article" },
};
...
export default function TerminalValuePage() {
  ...
  return (
    <DcfLessonShell number="03" href="/valuation/dcf/terminal-value" title="継続価値の計算方法――Terminal Valueを検証する" lead="明示予測期間後のFCFFをGordon Growth法で価値へ変換し、期末時点、割引、企業価値への集中度を確認します。" readingTime="約12分" sections={sections} previous={{ href: "/valuation/dcf/wacc", label: "02 WACCを計算する" }} next={{ href: "/valuation/dcf/sensitivity-analysis", label: "04 感応度を確認する" }}>
      <h2 id="definition">定義と使う場面</h2>
      <p>Terminal Value（継続価値）は明示予測期間後に事業が生むFCFFの価値です。Gordon Growth法は、最終予測年の翌年FCFFが一定率gで永久に成長すると仮定します。</p>
      <div className="formula">Terminal Value = FCFF(n+1) / (WACC − g)</div>
      <p>Exit Multiple法は補助的なクロスチェックに使えますが、基準ケースの算定は共有ケースのGordon Growth法だけを使い、外部のライブ市場マルチプルは使いません。</p>

      <h2 id="calculation">ステップ計算</h2>
      <ol><li>FY2030 FCFF：{finalFcff.toFixed(1)} 百万円</li><li>FCFF(n+1)：{finalFcff.toFixed(1)} × (1 + {(dcfCase.terminalGrowthRate * 100).toFixed(1)}%) = {nextFcff.toFixed(1)} 百万円</li><li>Terminal Value：{nextFcff.toFixed(1)} ÷ ({(valuation.wacc * 100).toFixed(2)}% − {(dcfCase.terminalGrowthRate * 100).toFixed(1)}%) = {valuation.terminalValue.toFixed(1)} 百万円</li><li>FY2030期末時点の継続価値を5期間割り引き、現在価値は{valuation.pvTerminalValue.toFixed(1)} 百万円です。</li></ol>
      ...
      <DcfCommonErrors errors={[{ title: "FCFF(n)を分子に使う", body: "継続価値は予測期間後の価値なので、最終年FCFFではなくFCFF(n) × (1+g)で求める翌年FCFFを使います。" }, { title: "期末価値を現在価値と混同する", body: "Terminal ValueはFY2030期末時点です。Enterprise Valueへ足す前に明示予測期間と同じWACCで現在価値へ割り引きます。" }]} />

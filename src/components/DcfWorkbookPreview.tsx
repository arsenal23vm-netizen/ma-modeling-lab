  ...
  return (
    <figure className="dcf-workbook-preview">
      ...
      <p className="mt-3 text-sm text-[#607080]">
        青いWACC前提は <code>入力!B19:B21</code>（株主資本構成比、負債構成比、永久成長率）で編集します。
      ...
      <div className="workbook-formula-bar" aria-label="数式バー">
        ...
        <code>=IF(前提!$B$11&lt;=前提!$B$12,NA(),B11/(前提!$B$11-前提!$B$12))</code>
      </div>
      <div className="data-scroll">
        <table className="data-table workbook-preview-sheet min-w-[840px]">
          ...
          <tbody>
            ...
            <tr><th scope="row" className="row-number">12</th><th scope="row">継続価値</th><td className="workbook-formula number" colSpan={6}>{money(valuation.terminalValue)}</td></tr>
            ...
            <tr><th scope="row" className="row-number">24</th><th scope="row" colSpan={7}>Enterprise Value 感応度分析</th></tr>
            ...
            {sensitivity.map((row, index) => <tr key={row.wacc}><th scope="row" className="row-number">{26 + index}</th><td /><th scope="row" className="workbook-formula number">{(row.wacc * 100).toFixed(1)}%</th>{row.cells.map((cell) => <td className="workbook-formula number" key={cell.terminalGrowthRate}>{cell.enterpriseValue === null ? "該当なし" : money(cell.enterpriseValue)}</td>)}</tr>)}
          </tbody>
        </table>
      </div>
      <ul className="workbook-sheet-tabs" aria-label="ワークシートタブ">
        {["表紙", "入力", "前提", "PL", "BS", "CF", "計算明細", "DCF", "チェック"].map((sheet) => (
